...
export default function FeedbackCard({ feedback, isEven }: { feedback: Feedback; isEven: boolean }) {
  const stars = Array.from({ length: 5 }, (_, i) => i < feedback.rating)

  if (isEven) {
    return (
      <div className="flex-shrink-0 w-full md:w-[600px] lg:w-[850px] bg-white grid grid-cols-1 md:grid-cols-[1fr_auto] gap-4">
        <div className="flex flex-col justify-between p-4 pb-8">
          <div className="">
            <div className="space-y-3 h-24 bg-brand-orange text-white px-6 py-5">
              <div className="font-medium text-sm">{feedback.name}, {feedback.title}</div>
              <div className="flex gap-1">
              {stars.map((filled, i) => (
                <FaStar key={i} className={filled ? 'text-white' : 'text-black/40'} size={16} />
              ))}
            </div>
            </div>
          </div>
